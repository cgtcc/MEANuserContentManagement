...
module.exports = function(router) {
        ...
        if (req.body.username === null || req.body.username === '' || req.body.password === null || req.body.password === '' || req.body.email === null || req.body.email === '' || req.body.name === null || req.body.name === '') {
            ...
            user.save(function(err) {
                if (err) {
                    ...
                    var email = {
                        from: 'AwesomeApp Staff, carl.gaudreault@outlook.com',
                        to: user.email,
                        subject: 'Your Activation Link',
                        text: 'Hello ' + user.name + ', thank you for registering at localhost.com. Please click on the following link to complete your activation: http://127.0.0.1/activate/' + user.temporarytoken,
                        html: 'Hello<strong> ' + user.name + '</strong>,<br><br>Thank you for registering at localhost.com. Please click on the link below to complete your activation:<br><br><a href="http://127.0.0.1/activate/' + user.temporarytoken + '">http://127.0.0.1/activate/</a>'
                    };
    ...
    router.post('/checkusername', function(req, res) {
        User.findOne({ username: req.body.username }).select('username').exec(function(err, user) {
            if (err) {
                ...
                var email = {
                    from: 'AwesomeApp Staff, carl.gaudreault@outlook.com',
                    to: 'carl.gaudreault@outlook.com',
    ...
    router.post('/authenticate', function(req, res) {
        var loginEmail = (req.body.email).toLowerCase(); // Ensure username is checked in lowercase against database
        User.findOne({ email: loginEmail }).select('username password active').exec(function(err, user) {
            if (err) {
                // Create an e-mail object that contains the error. Set to automatically send it to myself for troubleshooting.
                var email = {
                    from: 'AwesomeApp Staff, carl.gaudreault@outlook.com',
            ...
            if (err) {
                ...
                user.save(function(err) {
                    if (err) {
                        ...
                        var email = {
                            from: 'Staff Name, email@address.com',
                            to: user.email,
                            ...
                            html: 'Hello<strong> ' + user.name + '</strong>,<br><br>You recently requested a new account activation link. Please click on the link below to complete your activation:<br><br><a href="http://127.0.0.1/activate/' + user.temporarytoken + '">http://link.com/activate/</a>'
                        };
        ...
        User.findOne({ email: req.params.email }).select('email name username').exec(function(err, user) {
            if (err) {
                ...
                if (!user) {
                    ...
                    var email = {
                        from: 'Localhost Staff, carl.gaudreault@outlook.com',
                        to: user.email,
                    ...
                    user.save(function(err) {
                        if (err) {
                            ...
                            var email = {
                                ...
                                text: 'Hello ' + user.name + ', You recently request a password reset link. Please click on the link below to reset your password:<br><br><a href="http://127.0.0.1/reset/' + user.resettoken,
                                html: 'Hello<strong> ' + user.name + '</strong>,<br><br>You recently request a password reset link. Please click on the link below to reset your password:<br><br><a href="http://127.0.0.1/reset/' + user.resettoken + '">http://127.0.0.1/reset/</a>'
                            };
                            // Function to send e-mail to the user
                            client.sendMail(email, function(err, info) {
                                if (err) {
    ...
    // Route to get the currently logged in user using token 
